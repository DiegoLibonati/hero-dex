...
import { HEROE_TWO_MOCK, HEROES_MOCK } from "../../../tests/jest.setup";

type RenderComponent = {
  container: HTMLElement;
};

const hero = HEROE_TWO_MOCK;
...
test("It should render the hero's powerstats.", async () => {
  ...
  const strength = screen.getByText(`Strength: ${hero.powerstats.strength}`);
...
test("It should render the hero's appearance.", async () => {
  ...
  const eyeColor = screen.getByText(`Eye color: ${hero.appearance.eyeColor}`);
...
test("It should render the hero's biography.", async () => {
  ...
  const alterEgos = screen.getByText(`alter egos: ${hero.biography.alterEgos}`);
  ...
  const publisher = screen.getByText(`publisher: ${hero.biography.publisher}`);
  const alignment = screen.getByText(`alignment: ${hero.biography.alignment}`);
...
test("It should render the hero's works.", async () => {
  ...
  const occupation = screen.getByText(`occupation: ${hero.work.occupation}`);
